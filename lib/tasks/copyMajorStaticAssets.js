module.exports = async function(site, options = {}) {
  ...
  try {
    ...
    if (options.event === 'change') {
      ...
      assets = matchGlob(`${STATIC}/+(js|css)/**/*`);
    ...
    let cacheTypeBase = sanitizeFilename(`postcss${JSON.stringify(postCSSPlugins)}`);
          ...
          if (cached) {
            ...
            result = postcss(
              [
                ...postCSSPlugins.map(
                  plugin => {
                    const pluginPath = (pluginName) => require.resolve(pluginName, { paths: [process.cwd()] });

                    if (typeof plugin === 'object') {
                      // just requiring the plugin doesn't work when the package is linked
                      return require(pluginPath(plugin.name))(plugin.options);
                    } else {
