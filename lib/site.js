...
class Site {
  ...
  async prepareSite() {
    ...
    for (const [index, pageContentsBuffer] of contents.entries()) {
      ...
      if (!renderedMd) {
        renderedMd = this.renderMd(pageContents);
  ...
  loadData() {
    this._data = loadData(this.getLogger());
  }
  ...
  renderMd(contents) {
    let renderedMd = this._cache.getCached('markdown', contents);

    if (!renderedMd) {
      renderedMd = this._mdRenderer.renderMarkdown(contents);

      // set cache
      this._cache.setCache('markdown', contents, renderedMd);
    }
  ...
  async handlePostChange(event, post) {
    ...
    if (['add', 'change'].includes(event)) {
      ...
      let { html, meta } = this.renderMd(pageContents);
